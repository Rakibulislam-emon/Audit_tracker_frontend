...
export default function UniversalForm({
  ...
  const getRelationData = (fieldConfig) => {
    if (!fieldConfig.relation || !token) return { data: [], isLoading: false };
    ...
    // ✅ NEW: Relation Select Component
  const renderRelationSelect = (fieldKey, fieldConfig) => {
    const relationInfo = getRelationData(fieldConfig);
      ...
      <div className="relative">
        <select
          ...
          className={`
            ...
            transition-colors
            ${errors[fieldKey] ? "border-red-500 focus:ring-red-500" : "border-gray-300"}
        ...
        >
          ...
          {/* ✅ RELATION OPTIONS */}
          {!relationInfo.isLoading && relationInfo.data.map((item) => (
    ...
    switch (fieldConfig.type) {
      ...
      case "select":
        ...
        if (fieldConfig.relation) {
          return renderRelationSelect(fieldKey, fieldConfig);
        }
