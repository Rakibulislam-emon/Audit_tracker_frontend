...
export function RequirementsButton({ approval }) {
  ...
  const handleRequirementToggle = async (index, completed) => {
    setIsUpdating(true);
    try {
      await approvalService.updateRequirement(
        approval._id,
        index,
        completed,
        token
      ...
      toast.success(
        `Requirement ${completed ? "completed" : "marked incomplete"}`
      );
    } catch (error) {
        ...
        <DialogContent className="max-w-md">
              ...
              >
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    handleRequirementToggle(index, !!checked)
                  }
                  disabled={isUpdating}
                />
                <Label
                  ...
                  className={`text-sm flex-1 ${
                    req.completed ? "text-gray-500 line-through" : ""
                  }`}
                >
          ...
          <div className="flex justify-between items-center pt-4 border-t">
            <span className="text-sm text-gray-500">
              {requirements.filter((r) => r.completed).length} of{" "}
